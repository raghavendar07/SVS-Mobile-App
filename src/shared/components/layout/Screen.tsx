...
export function Screen({ header, footer, children }: ScreenProps) {
  return (
    <div className="flex h-full flex-col bg-white text-slate-900">
      {header && (
        <header className="sticky top-0 z-10 shrink-0 border-b border-slate-200 bg-white pt-safe-t">
          {header}
        </header>
      )}
      <main className="min-h-0 flex-1 overflow-y-auto">{children}</main>
      {footer && (
        <footer className="sticky bottom-0 z-10 shrink-0 border-t border-slate-200 bg-white pb-safe-b">
          {footer}
        </footer>
      )}
    </div>
